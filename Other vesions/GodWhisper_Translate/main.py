...
class App:
    def __init__(self):
        ...
        self._translation_word_buffer: list[str] = []
    ...
    def _build_ui(self) -> None:
        ...
        # ---- Live translation (Italian → English, every 10 words) ----
        trans2_outer, trans2_inner = self._panel(main, "LIVE TRANSLATION (Italian → English)")
        trans2_outer.pack(fill=tk.BOTH, expand=True, pady=(0, 8))
        btn_row2 = tk.Frame(trans2_inner, bg=BG_PANEL)
        btn_row2.pack(fill=tk.X, pady=(0, 4))
        ttk.Button(btn_row2, text="Clear", command=self._clear_translation).pack(side=tk.LEFT)
        self._translation_text = scrolledtext.ScrolledText(
            trans2_inner,
            wrap=tk.WORD,
            height=5,
            font=FONT_MONO,
    ...
    def _on_transcription(self, text: str) -> None:
        ...
        # Buffer words and translate every 10 words when language is Italian
        is_italian = "Italian" in self._language_var.get()
        if is_italian and text.strip():
            words = re.findall(r"\S+", text)
            self._translation_word_buffer.extend(words)
            while len(self._translation_word_buffer) >= 10:
                chunk = self._translation_word_buffer[:10]
                self._translation_word_buffer = self._translation_word_buffer[10:]
                phrase = " ".join(chunk)
                def do_translate(phrase_to_translate: str) -> None:
                    try:
                        result = GoogleTranslator(source="it", target="en").translate(phrase_to_translate)
                        if result:
                            self.root.after(0, lambda r=result: self._append_translation(r + " "))
                    except Exception as e:
                        self.root.after(0, lambda err=str(e): self._append_translation(f"[Translation error: {err}] "))
                threading.Thread(target=do_translate, args=(phrase,), daemon=True).start()
    ...
    def _append_translation(self, text: str) -> None:
        self._translation_text.insert(tk.END, text)
        self._translation_text.see(tk.END)

    def _clear_translation(self) -> None:
        self._translation_text.delete("1.0", tk.END)
        self._translation_word_buffer.clear()
